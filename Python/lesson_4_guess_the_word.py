...
def guess_the_word():
    base_words = ['год', 'человек', 'время', 'дело', 'жизнь', 'день', 'рука', 'раз', 'работа', 'слово', 'место', 'лицо',
    ...
    --------
    ''']
    random_word = [*choice(base_words)]
    false_attempt = 0
    true_attempt = 0
    used_words = []

    print('''Спаси бедолагу, угадай слово отгадывая по буквам. 
За каждую ошибку будет добавляться часть бедолаги, 
и как только будут собраны все части - ты проиграешь''')
    print('\nТекущее слово состоит из', len(random_word), 'букв')
    guessed_word = [*(len(random_word) * '-')]
    
    while false_attempt != len(gallow) - 1:
        print(gallow[false_attempt])
        user_char = input('Введи букву: ')
        while not user_char.isalpha() and len(user_char) != 1:
            user_char = input('Принимается только одна буква, введитте снова: ')
        if user_char in random_word:
            true_attempt += 1
            print('Список не подошедших букв: ', *used_words)
            for i in range(len(random_word)):
                if random_word[i] == user_char:
                    guessed_word[i] = user_char

            print('Ты угадал: ', ''.join(guessed_word))
            if guessed_word == random_word:
                print("Ты отгадал все буквы, поздравляю!")
                return
            elif len(random_word)/2 <= true_attempt and input('Хочешь ввести слово полностью? (д/н): ').lower() == 'д':
                if input('Пиши: ').lower() == ''.join(random_word):
                    print("Ты прав! Это: ", ''.join(random_word))
                    return
            else:
                continue
        false_attempt += 1
        used_words.append(user_char)
        print('Не угадал: ', ''.join(guessed_word))
        print('Список не подошедших букв: ', *used_words)

    print('Ты проиграл! Слово было:', ''.join(random_word))
    print(gallow[false_attempt])
    return
